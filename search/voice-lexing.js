...
const Word = createToken({ name: "Word", pattern: /([([]?(\|?[._@'0-9a-zäöüß](?!:)(?![a-z]:)([+*?]|\{[0-9],?([0-9]+)?\})?)+([\])]([+*?]|\{[0-9],?([0-9]+)?\})?)?)+/}) // [._@0-9a-zäöüß\])*?+]
const Pos = createToken({ name: "Pos", pattern: /([([]?(\|?f?[.A-Z]([+*?]|\{[0-9],?([0-9]+)?\})?)+([\])]([+*?]|\{[0-9],?([0-9]+)?\})?)?)+/}) // [.A-Z\])*?+]
