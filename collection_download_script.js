...
if (require.main === module) {
    try {
        main(parseArgs(["https://arche.acdh.oeaw.ac.at/api/171833", "--recursive", "--skipUrl", "https://arche.acdh.oeaw.ac.at/api/171860"], argsDefs));
    } catch (err) {
        /** @type {Error} */
        const e = err;
...
async function getFilename(url) {
    ...
    for (let l of resp.split('\n')) {
        ...
        if (l.startsWith(locationProp)) {
            location = l.slice(locationProp.length + 2, -4)
            logger.info(l)
        }
        if (l.startsWith(filenameProp)) {
            filename = l.slice(filenameProp.length + 2, -4)
